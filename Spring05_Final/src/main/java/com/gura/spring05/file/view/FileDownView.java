 ...
 * - @Component("bean의 이름 지정") 어노테이션을 붙여서 이름이 있는 bean이 되게한다.
 * - servlet-context.xml 에 BeanNameViewResolver 설정을 추가한다.
 * 
 *  @Component(이름지정)하는 이유
 *  - 예를 들어 FileDao타입이 Spring Bean Container에 여러개일 때 서비스에 구분해서 연결해주기 위해.
 *  Dao 타입이 OracleFileDaoImpl, MySqlFileDaoImpl일 때 @Repository만 붙어있다면 두개가 들어오기때문.
 */
...
public class FileDownView extends AbstractView {
	...
	protected void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request,
		...
		// request.getServletContext() = application 어플리케이션은 jsp에선 기본객체지만 여긴 없음 download.jsp랑 비교
